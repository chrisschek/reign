...
public class DefaultMetricsService extends AbstractService implements MetricsService {
    ...
    public static final int DEFAULT_AGGREGATION_INTERVAL_MILLIS = 15000;

    /** interval btw. aggregations at service level */
    private volatile int aggregationIntervalMillis = DEFAULT_AGGREGATION_INTERVAL_MILLIS;
                ...
                try {
                    synchronized (exportMeta) {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug("EXPORTED METRICS (export):  clusterId={}; serviceId={}; path={}; data={}",
                                    clusterId,
                                    serviceId, exportMeta.dataPath,
                                    sb.toString().replace("\n", ""));
                        }
    ...
    public void setAggregationIntervalMillis(int aggregationIntervalMillis) {
        if (aggregationIntervalMillis < 1000) {
            throw new ReignException("aggregationIntervalMillis is too short:  aggregationIntervalMillis="
    ...
    synchronized void scheduleRotator() {
        Runnable rotatorRunnable = new RotatorRunnable();
        executorService.scheduleAtFixedRate(rotatorRunnable, 250, 250, TimeUnit.MILLISECONDS);
    }
            ...
            try {
                for (Entry<String, ExportMeta> mapEntry : exportPathMap.entrySet()) {
                    ...
                    synchronized (exportMeta) {
                        ...
                        if (exportMeta.dataPath != null) {
                            String oldPath = exportMeta.dataPath;
                            ...
                            try {
                                ...
                                zkClientUtil.updatePath(getContext().getZkClient(), getContext().getPathScheme(),
                                        oldPath, sb.toString().getBytes(UTF_8), getContext()
                                                .getDefaultZkAclList(), CreateMode.PERSISTENT, -1);

                                if (logger.isDebugEnabled()) {
                                    logger.debug(
                                            "EXPORTED METRICS (rotating):  currentMetricRegistry={}; workingMetricRegistry={}; path={}; data={}",
                                            currentMetricRegistry, workingMetricRegistry, oldPath, sb
                                                    .toString().replace("\n", ""));
                                }
                            } catch (Exception e) {
    ...
    synchronized void scheduleCleaner() {
        ...
        executorService.scheduleAtFixedRate(cleanerRunnable, this.aggregationIntervalMillis,
                Math.max(this.aggregationIntervalMillis * 2, 60000), TimeUnit.MILLISECONDS);
    }
    ...
    synchronized void scheduleAggregation() {
        ...
        aggregationFuture = executorService.scheduleAtFixedRate(aggregationRunnable,
                0, this.aggregationIntervalMillis, TimeUnit.MILLISECONDS);
    }
    ...
    long millisToExpiry(MetricsData metricsData, long currentTimestamp) {
        if (metricsData == null || metricsData.getIntervalLengthUnit() == null
            ...
            logger.trace("millisToExpiry():  exiting early:  metricsData={}; intervalLengthUnit={}; intervalLength={}",
                    metricsData, metricsData != null ? metricsData.getIntervalLengthUnit() : null,
                    metricsData != null ? metricsData.getIntervalLength() : null);
            return -1;
        }
        long intervalLengthMillis = metricsData.getIntervalLengthUnit().toMillis(metricsData.getIntervalLength());
        long result = metricsData.getIntervalStartTimestamp() + intervalLengthMillis - currentTimestamp;
        logger.trace(
                "millisToExpiry():  intervalStartTimestamp={}; intervalLengthMillis={}; currentTimestamp={}; result={}",
                metricsData.getIntervalStartTimestamp(), intervalLengthMillis, currentTimestamp, result);
        return result;
    }
